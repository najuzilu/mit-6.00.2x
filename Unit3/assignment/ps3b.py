...
class Patient(object):
	...
	def update(self):
		...
		for i in range(len(self.viruses)):
			...
			if virus.doesClear():
				removeViruses.append(virus)
			else:
				popDensity = self.getTotalPop() / self.maxPop * virus.maxBirthProb
				if popDensity < 1.0:
					try:
						...
						continue
		for each in removeViruses:
			self.viruses.remove(each)
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
	...
	results = [ [None for _ in range(numTrials)] for _ in range(timesteps)] 
	for t in range(numTrials): 
		...
		for i in range(timesteps):
			...
			results[i][t] = totalPop

	final = []
	for row in results:
		final.append(sum(row)/len(row))

	pylab.plot(final, label = "SimpleVirus")
